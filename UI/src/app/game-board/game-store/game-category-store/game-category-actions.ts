...
import { CategoryInstance, GameCategory } from 'src/app/models/category';

export const sessionCategoryActions = createActionGroup({
    source: 'Session Categories',
    events: {
        'Set Selected category': props<{ id: string }>(),
        'Load Categories': props<{ categories: CategoryInstance[] }>(),
        'Load Categories Failure': (error: any | Error) => ({ error }),
