import { GameData } from "../models/game";

export const games: Array<GameData> = [
    {
        id: 'd12adfa5-26b5-4a6b-bd65-2dd02768437f',
        name: 'Game One',
        valueSymbol: '$',
        rounds: [
            {
                id: '43955bc2-06ca-4402-b52a-2ca27bb3bebc',
                type: 'Single',
                categoryies: [
                    {
                        id: 'd6eea9cf-8970-4ad6-bc40-aa08c265656e',
                        name: 'Sabbats',
                        questions: [
                            {
                                id: '58484667-2208-46f4-ba96-0cd2b166a39d',
                                value: 100,
                                ask: 'This fire Sabbat celebrates the union of the God and the Goddess',
                                answer: 'Beltane'
                            },
                            {
                                id: '20c9cc0d-317f-4658-811d-986c9dc9c4c0',
                                value: 200,
                                ask: 'This Sabbat celebrates the dark half of the year relinquishing to the light half',
                                answer: 'Yule'
                            },
                            {
                                id: '3171a860-b594-4f75-8ccb-153b8b653650',
                                value: 300,
                                ask: 'One of the names for this Sabbat is a contraction for “Loaf Mass”',
                                answer: 'Lammas'
                            },
                            {
                                id: 'f17cd298-c728-4540-8b5f-7b383e243dad',
                                value: 400,
                                ask: 'This lesser Sabbat  is sometimes likened to “Pagan Thanksgiving”',
                                answer: 'Mabon'
                            },
                            {
                                id: 'b24f3d67-9126-4639-b9fa-e6eb0412734d',
                                value: 500,
                                ask: 'Catholics celebrate the feast  of St. Blaise at this fire Sabbat',
                                answer: 'Imbolc'
                            }
                        ]
                    },
                    {
                        id: '670bfd57-7440-46eb-8693-76b1b8481c0a',
                        name: 'Mythological Monsters',
                        questions: [
                            {
                                id: 'bc05d953-1f39-4056-875b-a0d5d5733cc4',
                                value: 100,
                                ask: 'This mythological guardian ate any who answered her riddles incorrectly',
                                answer: 'The Sphynx'
                            },
                            {
                                id: '689fbc5f-88fd-471f-b37a-61bcc17dc9a0',
                                value: 200,
                                ask: 'Although her beauty was great, a stare from her could turn anyone to stone',
                                answer: 'Medusa'
                            },
                            {
                                id: '37ad49ed-bf4e-4c11-8ee5-14685acbd3d2',
                                value: 300,
                                ask: 'Though its gaze can turn victims to stone, it finds the crow of a rooster fatal',
                                answer: 'Basilisk',
                                alternatives: ['Cockatrice']
                            },
                            {
                                id: 'e4f87f91-c33e-4986-be38-c58312ace22f',
                                value: 400,
                                ask: 'This Egyptian “Devourer of Souls” had the head of a crocodile, the front of a lion, and the tail end of a hippopotamus',
                                answer: 'Ammit',
                            },
                            {
                                id: 'f7cc9963-f60c-4a66-8f1d-0238b4443383',
                                value: 500,
                                ask: 'This 8-legged son of Loki could run faster than any other creature, including in the air or on water',
                                answer: 'Sleipnir'
                            }
                        ]
                    },
                    {
                        id: '90e3ae37-0b55-42c7-8bda-585b10c9695f',
                        name: '“A” Rolling Stone',
                        questions: [
                            {
                                id: '8c57c6e9-8c1f-4911-872c-6739d1f36c09',
                                value: 100,
                                ask: 'This powerhouse of a yellow stone didn`t start out that way-it was liquid originally',
                                answer: 'Amber'
                            },
                            {
                                id: '18609b15-bf2b-4042-b43d-602733620aae',
                                value: 200,
                                ask: 'This purple stone shouldn\'t take the place of a doctor, but it is good for healing',
                                answer: 'Amethyst'
                            },
                            {
                                id: 'bb71a25a-3f99-4bcc-b954-b0660a4a3565',
                                value: 300,
                                ask: 'You\'d have to go “South of the border” for this green healing stone, but it will keep you calm on the rough trip back up',
                                answer: 'Amazonite'
                            },
                            {
                                id: '4f26c3aa-ad75-4fe2-8170-45bb90873dc4',
                                value: 400,
                                ask: 'This quartz, commonly forest green, strengthens your blood & muscle tissues, helps your libido, and releases fear and anxiety',
                                answer: 'Aventurine'
                            },
                            {
                                id: '123db793-3bd4-4fd6-9508-67f093609277',
                                value: 500,
                                ask: 'Legend says that this blue stone comes from the treasure chests of mermaids, and is therefore lucky for sailors',
                                answer: 'Aquamarine'
                            }
                        ]
                    },
                    {
                        id: 'c1164106-b693-4424-8ce9-6cc6cc35cdff',
                        name: 'The Fae',
                        questions: [
                            {
                                id: '9a487faa-6dd5-4ce0-879f-9a81826fb7c3',
                                value: 100,
                                ask: 'These horse-like creatures have sharp teeth and will drown their victims',
                                answer: 'Kelpies'
                            },
                            {
                                id: 'e09d82e3-156c-45df-b243-79dcd71248ec',
                                value: 200,
                                ask: 'An Irish death faery, this woman of the hills has flame red eyes from her constant weeping and wailing',
                                answer: 'Bean Sidhe',
                                note: 'pronounced "banshee"'
                            },
                            {
                                id: '3effe6d8-14dd-4492-8728-4626d088cbc3',
                                value: 300,
                                ask: 'This Celtic shape-shifter is able to shed her seal-skin and walk in the guise of a human',
                                answer: 'Selkie',
                                alternatives: ['Roane'],
                                note: 'It`s called a selkie in the Orkney and Shetland Islands, and a roane in Ireland'
                            },
                            {
                                id: '8ea3b32b-3eda-410c-9e1d-3571f6e01207',
                                value: 400,
                                ask: 'These are fae children that are left in the place of a stolen human baby',
                                answer: 'Changelings'
                            },
                            {
                                id: '669517b1-1ab4-4e56-9706-eee94c381627',
                                value: 5000,
                                ask: 'Most medieval stores of faerie and human interaction generally involve abductions & thus may be tied to this fae “tradition”, wherein human souls are chased as prey',
                                answer: 'The Wild Hunt'
                            }
                        ]
                    },
                    {
                        id: '9b5bc7a7-b4b2-46b8-8ac1-f93e5cb89b09',
                        name: 'Ancient Architecture',
                        questions: [

                            {
                                id: 'dcb2b008-38be-49c6-987d-1ea11cc2b1e9',
                                value: 100,
                                ask: 'This ancient structure was said to be the only manmade object that can be seen from space',
                                answer: 'The Great Wall of China'
                            },
                            {
                                id: '4124546f-5321-472a-a9a2-7872f52ab393',
                                value: 200,
                                ask: 'According to some myths, this ancient structure was the birth place of all the world`s languages',
                                answer: 'The Tower of Babel'
                            },
                            {
                                id: 'b18e32e2-c796-4130-b147-86193ba8c5e3',
                                value: 300,
                                ask: 'The Great Pyramid at Giza points directly at this constellation',
                                answer: 'Orion'
                            },
                            {
                                id: '76ebf4f8-41a3-44f0-80e2-f676d31314d1',
                                value: 400,
                                ask: 'The Statue of Liberty has three references to this ancient statue',
                                answer: 'the Colossus of Rhodes',
                                note: 'The Statue of Liberty is the same height, has solar rays on her head, and the poem she carries refers to the Colossus',
                            },
                            {
                                id: '485411ca-dfeb-43c5-91ab-0a3d5fa72c9a',
                                value: 500,
                                ask: 'The shadow of a serpent crawls up it every equinox',
                                answer: 'the Temple of Kukulcan',
                                alternatives: ['Temple of Quetzalcoatl', 'El Castillo'],
                                note: 'The Feathered shadow of Kukulcan, or Quetzalcoatl can be seen at the rising and setting of the sun along the side of the North staircase'
                            }
                        ]
                    }
                ]
            },
            {
                id: 'ea1d0e97-88ed-4244-9eb3-ea6ee73577d1',
                type: 'Double',
                categoryies: [
                    {
                        id: '537e2ef1-64cd-4d99-8e5b-0f54cdb3a5e7',
                        name: 'Making a Splash',
                        questions: [
                            {
                                id: 'b77fb1ea-9b1b-4322-972c-315549a8b190',
                                value: 200,
                                ask: 'The alluring voices of these beauties have been known to be the cause of many Sailors’ deaths',
                                answer: 'Sirens',
                                alternatives: ['Mermaids']
                            },
                            {
                                id: '8e2bd8ee-fb73-45a9-ba24-c9bf6d228257',
                                value: 400,
                                ask: 'Oddly enough, I\'m also the Greek God of Horses',
                                answer: 'Poseidon',
                                alternatives: ['Neptune'],
                                note: 'Neptune is the Roman name, not Greek'
                            },
                            {
                                id: 'fee39768-5f4e-41ef-a434-071438415192',
                                value: 600,
                                ask: 'This Norse God held a feast for the Aesir under the sea',
                                answer: 'Aegir'
                            },
                            {
                                id: '86a7ccd9-41d1-4fea-b24f-140af68ce6ee',
                                value: 800,
                                ask: 'These Greek demigods of the Mediterranean were known to steal children',
                                answer: 'Who are the Nereids'
                            },
                            {
                                id: '5ff250ac-95e4-4d26-9a11-5a331dbe524e',
                                value: 1000,
                                ask: 'Goddess of the sea & its creatures, she was sought by shamans for the release of seals for hunting',
                                answer: 'Sedna'
                            }
                        ]
                    },
                    {
                        id: '181a6157-1c47-4e57-823f-d547ad7bd18c',
                        name: 'A Match Made in Heaven',
                        questions: [
                            {
                                id: 'e8ea9b0c-2dd0-4516-b094-4040edc38603',
                                value: 200,
                                ask: 'From forth the fatal loins of these two foes, a pair of star-crossed lovers take their life',
                                answer: 'Who are Romeo and Juliet',
                                alternatives: ['Capulet and Montague']
                            },
                            {
                                id: '7f61249a-8e75-411a-af82-f15a010563c9',
                                value: 400,
                                ask: 'The patrons of Memphis, he created the nine gods, and she was to destroy mankind',
                                answer: 'Who are Ptah and Sekhmet'
                            },
                            {
                                id: '230d654b-1c51-46f8-a26d-16acf473bfa7',
                                value: 600,
                                ask: 'When her beloved Baldr was slain, this goddess of joy died of a broken heart',
                                answer: 'Who is Nanna'
                            },
                            {
                                id: '12d5fa9c-3175-448b-9ad1-ee5f34499b04',
                                value: 800,
                                ask: 'Kamapua\'a thought he lost this unwilling lover to a stream of lava',
                                answer: 'Who is Pele',
                                note: 'pronouncd [Kam-a-poo ah ah]'
                            },
                            {
                                id: '5970129b-de08-432c-8b27-abb18e9e27d7',
                                value: 1000,
                                ask: 'A marital spat between this Japanese pair caused the cycle of life and death for all living things',
                                answer: 'Who are Izanami and Izanagi'
                            }
                        ]
                    },
                    {
                        id: '52b12ccb-62dd-4463-98b3-e782e4c3f842',
                        name: 'Rising Stars',
                        questions: [
                            {
                                id: '0ddf0d94-73a0-4b42-b95c-0be488ed937f',
                                value: 200,
                                ask: 'This planet symbolizes the forces of deep transformation, and thus is associated with death',
                                answer: 'What is Pluto'
                            },
                            {
                                id: '38b76464-bae9-4293-9879-83c813fdc298',
                                value: 400,
                                ask: 'This is when a planet appears to be moving backwards when observed from the Earth',
                                answer: 'What is a planetary retrograde'
                            },
                            {
                                id: 'bb0991a7-e4db-4b41-8283-2f69ced18de5',
                                value: 600,
                                ask: 'It is when the Sun reaches the same degree it was in at the time of your birth',
                                answer: 'What is Solar Return'
                            },
                            {
                                id: '4e1941ca-d413-453d-8f95-07f1b84a12a1',
                                value: 800,
                                ask: 'You should avoid magic during this time of lunar movement between zodiac houses',
                                answer: 'What is the void of course'
                            },
                            {
                                id: '2ca7c778-6739-4ab7-a4a0-34392f509c89',
                                value: 1000,
                                ask: 'Commonly called the Dragon\'s head and Dragon\'s tail, these sensitive points on the elliptic indicate where the moon crosses the north & south latitudes',
                                answer: 'What are the north & south nodes'
                            }
                        ]
                    },
                    {
                        id: '12606096-9cb4-4cae-ac6b-b58296e62152',
                        name: 'Lost Cities',
                        questions: [
                            {
                                id: 'b4c97560-d291-4c4d-a987-bf9922103ad6',
                                value: 200,
                                ask: 'This city-state supposedly fell because of an affair and an apple',
                                answer: 'What is Troy'
                            },
                            {
                                id: 'b9cd8345-9793-463b-b3e1-f6b71799849b',
                                value: 400,
                                ask: 'Among the first white settlements in America, its settlers disappeared without a trace in the 1580\'s',
                                answer: 'What is Roanoake'
                            },
                            {
                                id: '488588d4-e3bb-4dda-a644-8758b4aaa287',
                                value: 600,
                                ask: 'This lost city was hidden from the world in a network of vines high in the Andes until it was rediscovered in 1911',
                                answer: 'What is Machu Picchu'
                            },
                            {
                                id: '474255a4-a160-4dfc-9fcc-a495e5e1167b',
                                value: 800,
                                ask: 'These two cities were perfectly preserved during the infamous eruption of Mount Vesuvius',
                                answer: 'What are Pompeii and Herculaneum'
                            },
                            {
                                id: '9d50f70f-a4e4-4d81-92d7-2c1825d3ff28',
                                value: 1000,
                                ask: 'Already in ruins by their time, the Aztecs referred to this as “the Place of the Gods”',
                                answer: 'What is Teotihuacan',
                                note: 'Pronounced [Tay-oh-tee-wah-KAHN]'
                            }
                        ]
                    },
                    {
                        id: 'e0989a35-a762-469d-a351-bcedb98a67f4',
                        name: 'Clash of the Titans',
                        questions: [
                            {
                                id: '5499d4be-156b-4e5a-91ce-9a4c5871e5dd',
                                value: 200,
                                ask: 'When this Titan castrated his father, blood fell from the wound to the Earth, creating the Furies',
                                answer: 'Who is Cronus'
                            },
                            {
                                id: '2e52e3ea-3451-4c76-a1a4-ad8cdc27b83f',
                                value: 400,
                                ask: 'She gave birth to 6 of the 12 Olympians',
                                answer: 'Who is Rhea'
                            },
                            {
                                id: '3651d56d-0059-447e-ad08-8c2f72295b27',
                                value: 600,
                                ask: 'Athena was instrumental in freeing this Titan from his punishment',
                                answer: 'Who is Atlas'
                            },
                            {
                                id: '00937294-311d-4aa1-affd-0d961540659b',
                                value: 800,
                                ask: 'In response to Epimethus giving many gifts to animals, he gave men a gift that earned him Zeus\' wrath',
                                answer: 'Who is Prometheus'
                            },
                            {
                                id: '4b647e8d-66e4-4e7e-8dfd-8f83aeba1f33',
                                value: 1000,
                                ask: 'Mother of the Fates, she personified law & order',
                                answer: 'Who is Themis',
                                alternatives: ['Ananke', 'Nyx']
                            }
                        ]
                    }
                ]
            }
        ]
    },
    {
        id: '71807a01-8dc2-40b9-ac9c-6977c3371357',
        name: 'Game Two',
        valueSymbol: '$',
        rounds: [
            {
                id: '0e50f6f6-b82c-4742-8c49-835fee00d65b',
                type: 'Single',
                categoryies: [
                    {
                        id: '8ed12b92-a93d-4dba-bc31-822b3de90e88',
                        name: 'Ancient Architecture',
                        questions: [
                            {
                                id: 'd5e43069-85f5-4a0c-99ac-55242c0c5a91',
                                value: 100,
                                ask: 'This ancient structure is the only manmade object that can be seen from space',
                                answer: 'The Great Wall of China'
                            },
                            {
                                id: '59e3df4d-27cb-4bd7-813b-ecb2a5766887',
                                value: 200,
                                ask: 'According to some myths, this ancient structure was the birth place of all the world`s languages',
                                answer: 'The Tower of Babel'
                            },
                            {
                                id: 'e3b401a9-c740-41f3-89cf-2afe0f2a44dd',
                                value: 300,
                                ask: 'The Statue of Liberty has three references to this ancient statue',
                                answer: 'The Colossus of Rhodes',
                                note: 'The Statue of Liberty is the same height, has solar rays on her head, and the poem she carries refers to the Colossus'
                            },
                            {
                                id: 'd8360e08-a242-4c89-9dcb-4ba8cee7091f',
                                value: 400,
                                ask: 'The Great Pyramid at Giza points directly at this constellation',
                                answer: 'Orion'
                            },
                            {
                                id: 'bc4c0e71-917e-42a2-aae2-59b0342d41cd',
                                value: 500,
                                ask: 'The shadow of a serpent crawls up it every equinox',
                                answer: 'The Temple of Kukulcan',
                                alternatives: ['El Castillo'],
                                note: 'The Feathered shadow of Kukulcan, or Quetzalcoatl can be seen at the rising and setting of the sun along the side of the North staircase'
                            }
                        ]
                    },
                    {
                        id: 'e27039c1-b06e-4e98-ae59-c859e1f1a1e6',
                        name: 'Underworld Deities',
                        questions: [
                            {
                                id: '6b06d73a-3c7c-4ffc-bbb1-c3f5dda000f9',
                                value: 100,
                                ask: 'This underworld goddess was returned to the land of the living due to the extremity of her mother`s mourning',
                                answer: 'Persephone',
                                alternatives: ['Proserpina']
                            },
                            {
                                id: 'b63f3364-aad4-43b2-accb-305e34938153',
                                value: 200,
                                ask: 'He was also a god of crops, minerals and prosperity as well as the Underworld',
                                answer: 'Hades',
                                alternatives: ['Pluto']
                            },
                            {
                                id: '2c0abb23-432b-4f62-aa05-60a289a6929e',
                                value: 300,
                                ask: 'If your heart weighs as evil, Anubis would feed it to her',
                                answer: 'Ammit'
                            },
                            {
                                id: 'b01e9fa0-5da7-4ea2-8a88-ca89e5f92c67',
                                value: 400,
                                ask: 'These 10 Chinese underworld rulers judge the fate of a soul after death',
                                answer: 'The Yama-Kings'
                            },
                            {
                                id: 'dbbdc392-beaa-4b1c-a8f2-5ca0f8c2e92a',
                                value: 500,
                                ask: 'Inanna was forced to strip naked in order to visit her realm',
                                answer: 'Erishkigal'
                            }
                        ]
                    },
                    {
                        id: '418abaa2-3922-4d60-9f03-44f01f08a09d',
                        name: 'Mythical Places',
                        questions: [

                            {
                                id: '885df359-f7f7-4151-a543-43b475987f9e',
                                value: 100,
                                ask: 'This mythical utopia is rumored to be hidden in the Kunlun Mountains',
                                answer: 'Shangri-La'
                            },
                            {
                                id: '401a5a0a-5f08-4ced-be2e-7b9df6d5a27a',
                                value: 200,
                                ask: 'Juan Ponce de León died in his quest to discover this',
                                answer: 'The Fountain of Youth'
                            },
                            {
                                id: '979eb4f7-4597-4585-92c4-57a913c3492e',
                                value: 300,
                                ask: 'This place was believed to lie on the floor of the Indian Ocean',
                                answer: 'Lemuria'
                            },
                            {
                                id: '5cc04ecc-ce0a-4da9-9a17-ec154991bffb',
                                value: 400,
                                ask: 'This Mystical Isle was also known for its apples',
                                answer: 'Avalon'
                            },
                            {
                                id: '820039ef-7ae5-416d-af6f-59a3c131ad2d',
                                value: 500,
                                ask: 'Some scholars believe that references to this mythical city actually were a reference to a person instead of a place',
                                answer: 'El Dorado'
                            }

                        ]
                    },
                    {
                        id: 'f7d76093-b499-42ec-8213-d4b4651c38d1',
                        name: 'Sabbats',
                        questions: [
                            {
                                id: '5631911e-6287-4642-8f33-902e5b2aeb52',
                                value: 100,
                                ask: 'This Horn of Plenty, filled with the bounty of the season is associated with Mabon',
                                answer: 'The Cornucopia'
                            },
                            {
                                id: 'c4257b7d-0190-47d2-9257-92dc0b4b9c4e',
                                value: 200,
                                ask: 'This fire festival marks the mid point between winter and summer',
                                answer: 'Imbolc',
                                alternatives: ['Candlemas']
                            },
                            {
                                id: '395094c4-7451-46a7-9e86-7750f60c7f57',
                                value: 300,
                                ask: 'This Sabbat occurs at the same time of year as the Roman festival of Floralia',
                                answer: 'Beltane'
                            },
                            {
                                id: '10b11cb6-de29-47f9-83cd-eb58d60d82f5',
                                value: 400,
                                ask: 'This Sabbat is named after the Celtic deity who by legend established it in honor of his foster mother',
                                answer: 'Lughnassadh',
                                alternatives: ['Lammas']
                            },
                            {
                                id: '288ef5a9-9a81-430a-8645-9efe9a021b5a',
                                value: 500,
                                ask: 'In 1643 the British Parliament abolished this heathen holiday',
                                answer: 'Yule'
                            }
                        ]
                    },
                    {
                        id: 'de109484-dbd4-4c18-a5f7-e4185b678d03',
                        name: 'Carpe Cerevisi',
                        questions: [
                            {
                                id: '3ec622a8-0c5c-48ba-9952-83247729bc85',
                                value: 100,
                                ask: 'This brew is made from fermented honey',
                                answer: 'Mead'
                            },
                            {
                                id: 'eda8bcbd-60f6-4001-b1b3-90f009a4e696',
                                value: 200,
                                ask: 'This Olympian brew was reputed to grant immortality',
                                answer: 'Ambrosia'
                            },
                            {
                                id: '11b334b7-4e3d-4030-b665-1fe9c1cb624a',
                                value: 300,
                                ask: 'Egypt was spared when Sekhmet was tricked into drinking this',
                                answer: 'Beer colored red with ochre',
                                alternatives: ['Beer dyed red']
                            },
                            {
                                id: '5ea68428-b24f-42e1-9915-079e0b0d1bbf',
                                value: 400,
                                ask: 'He was the first recorded in the Old Testament to make alcohol',
                                answer: 'Noah'
                            },
                            {
                                id: '1194b5d0-fda5-4310-8870-ca17b785c1fe',
                                value: 500,
                                ask: 'This ancient Babylonian law code included laws regulating beer and beer parlors',
                                answer: 'The Code of Hammurabi'
                            }
                        ]
                    }
                ]
            },
            {
                id: '3207272f-aa1c-4e3c-b90b-502cad03db79',
                type: 'Double',
                categoryies: [
                    {
                        id: '073b3bd9-fbf3-4e0a-b232-3ac061c01510',
                        name: 'Wherever I May "Rome"',
                        note: 'This category is looking for the Roman name, not the Greek',
                        questions: [
                            {
                                id: '421ee45f-a485-4938-939e-1958050d14ba',
                                value: 200,
                                ask: 'Though not associated with healing, his symbol has been associated with doctors',
                                answer: 'Mercury',
                                note: 'This category is looking for the Roman name, not the Greek'
                            },
                            {
                                id: 'b5ccb629-3d2b-45e9-911c-5eec6233ee99',
                                value: 400,
                                ask: 'She is the Roman Goddess of Nature, Fertility, and Childbirth',
                                answer: 'Diana',
                                note: 'This category is looking for the Roman name, not the Greek'
                            },
                            {
                                id: 'f07f937a-6e6f-41ca-9153-cede909478e3',
                                value: 600,
                                ask: 'Some say he was born from a silver egg and had two types of arrows',
                                answer: 'Cupid',
                                note: 'This category is looking for the Roman name, not the Greek'
                            },
                            {
                                id: '7fcd29ee-e441-4b86-b98d-33930ebf7de8',
                                value: 800,
                                ask: 'The sun of Somnus, he was the God of Dreams',
                                answer: 'Morheus',
                                note: 'This category is looking for the Roman name, not the Greek'
                            },
                            {
                                id: 'ea706abd-f20b-401e-b659-516439001158',
                                value: 1000,
                                ask: 'They were the personification of charm and beauty in nature and in human life',
                                answer: 'Gratiae',
                                note: 'This category is looking for the Roman name, not the Greek'
                            }
                        ]
                    },
                    {
                        id: 'f0de5d11-7c85-461c-881a-fcd5c6e72634',
                        name: 'Divinational Forms',
                        questions: [
                            {
                                id: '32a91629-d9e6-4f0a-8d39-044e6a4b0784',
                                value: 200,
                                ask: 'This method merely requires any deck of cards',
                                answer: 'Cartomancy',
                                note: 'Tarot is the most common form of Cartomancy'
                            },
                            {
                                id: 'b0d4eb75-04b6-463e-a471-a51f930be488',
                                value: 400,
                                ask: 'This method always requires a campfire, candle, or some other form of flame',
                                answer: 'Pyromancy'
                            },
                            {
                                id: '947d23c4-9b34-4691-bac1-99706814b6a9',
                                value: 600,
                                ask: 'Using sticks, stones and bones to cast lots can be used to tell the future with this',
                                answer: 'Cleromancy'
                            },
                            {
                                id: '74241308-ecd7-46ab-b2a5-c6cff7d57c6f',
                                value: 800,
                                ask: 'This method of divination involves reading the outcome from tossed arrows',
                                answer: 'Belomancy'
                            },
                            {
                                id: 'a8182174-552c-4b3f-a773-5c0ba3463920',
                                value: 1000,
                                ask: 'This method revolves around the reading of tea leaves',
                                answer: ' Tasseomancy',
                                alternatives: ['Tasseography']
                            }
                        ]
                    },
                    {
                        id: 'e6b81ed5-d089-4793-93ac-a1df5b2cd881',
                        name: 'Mythological Monsters',
                        questions: [
                            {
                                id: '31073564-5cdd-4d35-826a-bf3d65f69863',
                                value: 200,
                                ask: 'Heracles wear the skin of this creature',
                                answer: 'The Nemean Lion'
                            },
                            {
                                id: '8e620b60-8d95-49e4-a670-db43981048fe',
                                value: 400,
                                ask: 'Its blood is prophesied to be the poison that will kill Thor',
                                answer: 'Jörmungandr',
                                alternatives: ['The Midgard Serpent']
                            },
                            {
                                id: '6ca1f1aa-42d9-4946-8633-e10f1bcf5521',
                                value: 600,
                                ask: 'Every nine years seven of Athens` finest youths and fairest maidens were delivered to him',
                                answer: 'The Minotaur'
                            },
                            {
                                id: '409e9f8f-cc3f-4801-8273-83decb4979de',
                                value: 800,
                                ask: 'He was responsible for eclipses when he managed to swallow Ra`s Sun Boat',
                                answer: 'Who is Apophis',
                                alternatives: ['Apep']
                            },
                            {
                                id: 'c452636c-ea65-49b3-a379-58f291bfac4f',
                                value: 1000,
                                ask: 'This Japanese demon\'s most distinguishing feature is a circular depression on top of its head that must remain filled with water lest he die',
                                answer: 'Kappa'
                            },
                        ]
                    },
                    {
                        id: '0af3c405-7b1e-4dea-8115-3cca222297c9',
                        name: 'Burning Times',
                        questions: [
                            {
                                id: '2e410fde-7e2a-4750-b67c-26088a098d69',
                                value: 200,
                                ask: 'This 15th tome was used to identify a witch',
                                answer: 'Malleus Maleficarum',
                                alternatives: ['Hammer of the Witches']
                            },
                            {
                                id: 'c1a74e5a-8914-4724-8aae-b09764bdb067',
                                value: 400,
                                ask: 'This French woman was executed for crimes including Witchcraft, Heresy, and dressing in the manner of a man',
                                answer: 'Joan of Arc'
                            },
                            {
                                id: '06417b68-adb8-4f67-9105-f574951e7ed1',
                                value: 600,
                                ask: 'This Salem man\'s last words were reputed to be "More Weight"',
                                answer: 'Giles Corey'
                            },
                            {
                                id: 'e19dfb71-5091-4f17-83bd-3f64b6891605',
                                value: 800,
                                ask: 'He issued the Papal Bull disbanding the prosecuted Order of the Knights Templar',
                                answer: 'Pope Clement V'
                            },
                            {
                                id: '074d5fda-e91c-438e-9ff6-5893a72d80c2',
                                value: 1000,
                                ask: 'In 1573 he was executed for witchcraft, lycanthropy, and "eating meat on a Friday"',
                                answer: 'Gilles Garnier',
                                alternatives: ['the Werewolf of Dole', 'The Hermit of St Bonnot']
                            }
                        ]
                    },
                    {
                        id: 'eddc2c16-a422-4c93-ae31-7cd9fd605e9a',
                        name: 'Potpourri',
                        questions: [
                            {
                                id: 'b12c1495-4cf3-476e-8032-bb895e5eefcf',
                                value: 200,
                                ask: 'This is the first Sunday after the full moon after the spring equinox',
                                answer: 'When is Easter'
                            },
                            {
                                id: 'a15c9b60-7633-4406-8af5-7a10fa80d7fa',
                                value: 400,
                                ask: 'What three signs of the Zodiac are associated with water',
                                answer: 'Cancer, Scorpio, and Pisces'
                            },
                            {
                                id: 'ced42b0a-1cd8-4d4b-9930-52d857c3a727',
                                value: 600,
                                ask: 'In Tarot, this energy is at the beginning of the cycle of three',
                                answer: 'Cardinal'
                            },
                            {
                                id: 'a33696e2-78dd-45fc-aee1-8c5a77e8ca51',
                                value: 800,
                                ask: 'This one of the seven magical tools is used to incite Astral Projection',
                                answer: 'Scourge'
                            },
                            {
                                id: '54c52c32-ee79-48ba-989a-eb5a6a70a5cb',
                                value: 1000,
                                ask: 'The tenth Sephirah of the Tree of Life, it receives influences from the others',
                                answer: 'Malkuth'
                            }
                        ]
                    }
                ]
            }
        ]
    }
]
